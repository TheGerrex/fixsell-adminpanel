...
export class PrintersRegisterComponent {
  ...
  scanSuccessHandler(event: any) {
    console.log(event);
    //if barcode value is the same as previous barcode value, do nothing
    //check if value already exists
    if (this.barcode.includes(event)) {
      return;
    } else {
      this.barcode += event + ', ';
    }
    //else, add barcode value to barcode variable and seperate with a comma
  }
