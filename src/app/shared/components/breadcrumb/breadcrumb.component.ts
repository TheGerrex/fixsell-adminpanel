...
@Component({
  ...
  template: `
    <nav>
      <div class="breadcrumb-route" *ngFor="let breadcrumb of breadcrumbs; let i = index">
        <a [routerLink]="breadcrumb.url" [class.active]="i === breadcrumbs.length - 1">{{ breadcrumb.label }}</a>
  ...
  constructor(
    ...
    ngOnInit(): void {
      ...
      const createBreadcrumbs = (url: string): Observable<Breadcrumb[]> => {
        ...
        // Skip the first segment for labels
        ...
        for (let i = 0; i < labelSegments.length; i++) {
          if (labelSegments[i].match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
            breadcrumbObservables[i] = this.printerService.getPrinterName(labelSegments[i]).pipe(
              map(name => {
                // console.log(`Printer name for UUID ${labelSegments[i]} is ${name}`);
                return {
      ...
      this.router.events.pipe(
        ...
        switchMap((event: NavigationEnd) => createBreadcrumbs(event.urlAfterRedirects))
