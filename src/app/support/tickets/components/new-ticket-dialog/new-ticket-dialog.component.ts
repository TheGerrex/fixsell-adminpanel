...
export class NewTicketDialogComponent {
  ...
  ngOnInit(): void {
    ...
    if (this.data) {
      ...
      const startTime = this.data.startTime.padStart(5, '0'); // Ensure leading zeros
      this.createTicketForm.patchValue({
        appointmentStartTime: new Date(this.data.appointmentStartDate),
        startTime: startTime,
      });
    }
  ...
  calculateEndDate() {
    ...
    if (startDateControl && startTimeControl && endTimeControl) {
      ...
      if (startTime !== null && endTime !== null) {
        ...
        this.createTicketForm.get('appointmentStartTime')?.setValue(startDateTime, { emitEvent: false });
        this.createTicketForm.get('appointmentEndTime')?.setValue(endDateTime, { emitEvent: false });

        console.log('Fecha de inicio', startDateTime);
        console.log('Fecha de terminacion', endDateTime);
      }
    }
  }
  ...
  submitForm() {
    ...
    if (this.createTicketForm.valid) {
      ...
      console.log('Submitting ticket:', ticket); // Log the ticket data
      ...
      this.ticketService.createTicket(ticket).subscribe(
        (response) => {
          ...
          this.dialogRef.close(ticket); // Close the dialog and pass the ticket as the result
        },
        (error) => {
          // handle error response
          console.error('Error creating ticket:', error); // Log the error response
